...
interface EventPositions {
  absMouse: Position;
  mouse: Position;
  holdMouseStart: Position | null;
  holdMouseEnd: Position | null;
  rmbHold: Position | null;
}
...
export default class AppEventsManager {
  ...
  readonly positions: EventPositions;
  readonly screenMovingSpeed: Position;
  ...
  mousemove(from: Position) {
    const to: Position = this.positions.mouse;
    const dl: Position = {
      x: to.x - from.x,
      y: to.y - from.y
    };
